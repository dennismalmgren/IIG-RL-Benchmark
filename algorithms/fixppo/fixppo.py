...
class FIXPPOAtariAgent(nn.Module):
    """A PPO Atari agent module."""

    def __init__(self, num_actions, observation_shape, device):
        super(FIXPPOAtariAgent, self).__init__()
...
class FIXPPO(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # FixPO arguments
        self.kl_target = kwargs.get("kl_target", 0.01)
        self.kl_tolerance = kwargs.get("kl_tolerance", 0.10)
        self.kl_beta_init = kwargs.get("kl_beta_init", 1.0)
        self.kl_beta_lr = kwargs.get("kl_beta_lr", 1e-3)
        self.max_fixup_epochs = kwargs.get("max_fixup_epochs", 10)

        self.log_beta = torch.nn.Parameter(torch.log(torch.tensor(self.kl_beta_init + 1e-8)))
        self.beta_opt = torch.optim.Adam([self.log_beta], lr=self.kl_beta_lr)
        self.beta = float(torch.nn.functional.softplus(self.log_beta).detach().cpu())
        ...
        #FixPO
        # store π_old(a|s) for each collected state (to compute forward KL exactly)
        self.policy_probs = torch.zeros(
            (self.steps_per_batch, self.num_envs, self.num_actions), dtype=torch.float32
        ).to(device)
    ...
    def step(self, time_step, is_evaluation=False):
        if is_evaluation:
            ...
            with torch.no_grad():
                ...
                # FixPO
                self.policy_probs[self.cur_batch_idx] = probs  # π_old(·|s) at collection time
    ...
    def learn(self, time_step):
        ...
        b_old_probs = self.policy_probs.reshape((-1, self.num_actions))
        def _forward_kl(old_probs, new_probs, eps=1e-8):
            # D_KL(π_old || π_new) per state
            return (old_probs * (torch.log(old_probs + eps) - torch.log(new_probs + eps))).sum(dim=-1)
        ...
        for _ in range(self.update_epochs):
            ...
            for start in range(0, self.batch_size, self.minibatch_size):
                ...
                # FixPO
                kl_mb = _forward_kl(b_old_probs[mb_inds], new_probs)   # exact forward KL
                kl_loss = kl_mb.mean()

                # --- Policy loss: NO ratio clipping ---
                pg_loss = -(ratio * mb_advantages).mean()
                ...
                # --- Final loss (FixPO): maximize surrogate - β * KL ---
                loss = pg_loss + self.beta * kl_loss + v_loss * self.value_coef - self.entropy_coef * entropy_loss
            ...
            #         break
                ...
                # Do FixUP updates
                # ---- Measure mean KL(old||new) over the whole on-policy batch ----
                with torch.no_grad():
                    kl_vals = []
                    for start2 in range(0, self.batch_size, self.minibatch_size):
                        end2 = start2 + self.minibatch_size
                        mb = slice(start2, end2)
                        _, _, _, _, new_probs_full = self.get_action_and_value(
                            b_obs[mb], legal_actions_mask=b_legal_actions_mask[mb], action=b_actions.long()[mb]
                        )
                        kl_vals.append(_forward_kl(b_old_probs[mb], new_probs_full))
                    mean_kl_tensor = torch.cat(kl_vals).mean()
                

                # --- after you compute mean_kl for the whole on-policy batch ---
                mean_kl_det = mean_kl_tensor.detach()  # important: no grad into θ
                beta = torch.nn.functional.softplus(self.log_beta)  # enforce β ≥ 0

                # Dual objective: maximize β * (mean_kl - kl_target)
                dual_loss = -(beta * (mean_kl_det - self.kl_target))  # negative for ascent with Adam

                self.beta_opt.zero_grad()
                dual_loss.backward()   # grads only flow to log_beta
                self.beta_opt.step()

                # cache a float β for logging/use in the primal loss
                self.beta = float(torch.nn.functional.softplus(self.log_beta).detach().cpu())

                # ================== Fix-up phase (guarantee the trust region) ==================
                def _mean_kl_now():
                    with torch.no_grad():
                        vals = []
                        for start3 in range(0, self.batch_size, self.minibatch_size):
                            end3 = start3 + self.minibatch_size
                            mb = slice(start3, end3)
                            _, _, _, _, np_full = self.get_action_and_value(
                                b_obs[mb], legal_actions_mask=b_legal_actions_mask[mb], action=b_actions.long()[mb]
                            )
                            vals.append(_forward_kl(b_old_probs[mb], np_full))
                        return torch.cat(vals).mean()
                

                cur_mean_kl = mean_kl_det

                kl_cap = (1.0 + self.kl_tolerance) * self.kl_target
                b_inds_copy = np.copy(b_inds)
                fix_epochs = 0
                while cur_mean_kl > kl_cap and fix_epochs < self.max_fixup_epochs:
                    np.random.shuffle(b_inds_copy)
                    for start in range(0, self.batch_size, self.minibatch_size):
                        end = start + self.minibatch_size
                        mb_inds = b_inds_copy[start:end]
                        # Minimize KL only
                        _, _, entropy, _, np_fix = self.get_action_and_value(
                            b_obs[mb_inds], legal_actions_mask=b_legal_actions_mask[mb_inds], action=b_actions.long()[mb_inds]
                        )
                        kl_fix = _forward_kl(b_old_probs[mb_inds], np_fix).mean() - self.entropy_coef * entropy.mean()
                        self.optimizer.zero_grad()
                        kl_fix.backward()
                        nn.utils.clip_grad_norm_(self.parameters(), self.max_grad_norm)
                        self.optimizer.step()
                    cur_mean_kl = _mean_kl_now()
                    fix_epochs += 1
