...
class PPOAgent(nn.Module):
    ...
    def get_action_and_value(self, x, legal_actions_mask=None, action=None, clip_probability_eps=None):
        ...
        # Replacement for CategoricalMasked
        logits = torch.where(legal_actions_mask.bool(), logits, self.mask_value)
        logits = logits - logits.logsumexp(dim=-1, keepdim=True)
        dist_probs = torch.nn.functional.softmax(logits, dim=-1)


        # if clip_probability_eps is not None:
        #     dist_probs_high = (dist_probs >= clip_probability_eps).detach()
        #     dist_probs_masked = dist_probs * dist_probs_high
        #     dist_probs_sum_mask = dist_probs_masked.sum(dim=-1) > 0
        #     dist_probs_masked[~dist_probs_sum_mask] = dist_probs[~dist_probs_sum_mask]
        #     dist_probs_masked[dist_probs_sum_mask] = dist_probs_masked[dist_probs_sum_mask] / torch.sum(dist_probs_masked[dist_probs_sum_mask], dim=-1, keepdim=True)
        #     dist_probs = dist_probs_masked
...
class PPO(nn.Module):
    ...
    def get_action_and_value(self, x, legal_actions_mask=None, action=None):
        return self.network.get_action_and_value(x, legal_actions_mask, action, self.prob_clip_eps)
    ...
    def learn(self, time_step):
        ...
        for _ in range(self.update_epochs):
            ...
            for start in range(0, self.batch_size, self.minibatch_size):
                ...
                loss = (
                    pg_loss
                    - self.entropy_coef * self.entropy_reg * entropy_loss
